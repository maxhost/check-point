...
| 0047 | 2026-08-30 | **Deuda de formato y CI rojo.** `pnpm format:check` falla con 20 archivos sin formatear y es el **primer** paso de `.github/workflows/ci.yml`, así que el CI corta ahí y `lint`/`typecheck`/`test`/`test:e2e`/`build` **nunca corren en GitHub** (3 corridas seguidas en `failure` sobre `main`). Formatear los 20, agregar el script `format` que falta en el root, y prevenir la recaída (reordenar gates y/o hook de formato). Sin cambio de comportamiento | borrador | sí | `specs/0047-deuda-de-formato-y-ci-rojo.md` |
