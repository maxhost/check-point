...
| 0039 | 2026-08-16 | Branding de la landing de enrolamiento (`/enroll/[programId]` + confirmación): logo del negocio (ruta pública existente, sin exponer la clave R2) + color de marca en los botones de acción (form + "Activar notificaciones" + acento del instructivo iOS) con texto por luminancia; helper puro `readableTextColor`; fallback al nombre si no hay logo. NO re-diseña `/wallet` (arco 0031); componentes compartidos brandeados solo con prop `accentColor` opcional. Sin migración ni secreto nuevo | cerrada | sí | `specs/0039-branding-de-la-landing-de-enrolamiento.md` |
