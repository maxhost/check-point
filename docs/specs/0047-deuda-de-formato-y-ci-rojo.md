...
disjunta: si
archivos: 20 archivos a formatear (lista abajo), package.json (script `format`), .github/workflows/ci.yml (orden de gates), .claude/hooks/ (opcional)
---
...
`.claude/settings.local.json` es config local no versionada como código fuente —
decidir si se formatea o se agrega a `.prettierignore`.
...
| Archivo | Acción |
|---|---|
| Los 20 de la tabla | formatear con `prettier --write` |
| `package.json` (root) | editar — agregar script `format` |
| `.github/workflows/ci.yml` | editar — reordenar gates (si se elige (a)) |
| `.claude/settings.json` | editar — hook de formato (si se elige (b)) |
| `.prettierignore` | crear/editar — sólo si se decide ignorar `settings.local.json` |
...
- [ ] `git diff` de los 20 archivos es **exclusivamente de formato**: cero cambios de
...
- [ ] Se aplicó una prevención (a, b o c) y está documentada.
