...
| 57 | **`core.business.status`** — que un negocio tenga estado propio | pedido del **owner** el 2026-09-17, al contestar las decisiones de la 0068 | **Requisito nuevo, no un bug.** Medido: esa columna **no existe** (`server/schema/business.ts`); los `status` que si existen son los de `business_membership`, `location` y `subscription`. Es **esquema nuevo** y la 0068 no toca esquema. **La otra mitad de lo que pidio el owner ya esta hecha**: toda alta inserta su suscripcion `plan:'free'`/`status:'active'` (`api/onboarding/business/route.ts:155`). Lugar natural: la **3ª spec del arco** (entitlements, `can()`/`limitOf()`), que es la que va a leer ese estado. **QUE FALTA, respondido el 2026-09-17: la columna es lo barato —una linea de esquema y una migracion—; lo que NO existe es la DECISION DE PRODUCTO.** Medido: **ningun doc del repo define que estados tendria ni que bloquea cada uno** (`rg` sobre `docs/adr` y las specs: los unicos hits son el `business_membership.status` del ADR 0044, que es otra cosa, y la mencion de la 0069 declarandolo fuera de alcance). O sea que faltan, en orden: **(1) el owner dice cuales son los estados y que corta cada uno** (p.ej. `active` / `suspended` / `closed`: ¿un negocio suspendido pierde el mostrador? ¿los pases de Wallet siguen vivos? ¿el staff puede entrar?), **(2) la spec**, **(3) la migracion**. Sin (1) no se puede escribir (2): es el caso exacto que el ADR 0071 manda resolver ANTES de la prosa |
