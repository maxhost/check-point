...
| 28 | Canal de actualización y push de Wallet | 0033 | pendiente (spec BORRADOR) | **PRÓXIMA FEATURE (2026-08-15) — prerequisito técnico duro de la spec 0031.** Stub/borrador, separado de la 0029 (2026-08-14). Web service REST de PassKit (registro de dispositivos + APNs) + `PATCH`/`addMessage` de Google + notificaciones scopeadas por destinatario (miembros de un negocio) + rotación/revocación del pase. Suma el secreto APNs (`.p8`). Depende de 0029 (hecha). **Primer paso de la próxima sesión: cerrar el diseño técnico con el owner** (proveedor/alcance de las notificaciones, qué dispara un push — el `order` de la 0030 es la fuente candidata) antes de escribir código, siguiendo el mismo patrón que 0036/0034 antes de sus specs consumidoras. |
