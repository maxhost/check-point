...
| 16 | Implementar registro y autenticación real de Owner | 0022 | en curso | Spec cerrada; Neon + Better Auth + Stripe Checkout + negocio/local. Credenciales se cargan de forma segura durante despliegue. **Bloqueo de dependencias (2026-08-11) RESUELTO:** codex bajo Auto no podía instalar paquetes (sandbox sin red + store de pnpm fuera del workdir, ver CLAUDE.md §Gotchas). Fix: `storeDir: .pnpm-store` en `pnpm-workspace.yaml` (gitignored, pre-cargado con `pnpm fetch`) — `pnpm install --offline` verificado end-to-end (268 paquetes, 0 descargas) + `pnpm run typecheck` real, 3/3 verdes. Lockfile reconciliado con las 3 deps que ya estaban en package.json (drizzle-kit/drizzle-orm/@neondatabase/serverless) sin agregar nada nuevo. Falta re-`pnpm fetch` si se suman deps nuevas antes de la próxima sesión de codex bajo Auto |
