...
Ultima actualizacion: 2026-08-16 (**spec 0039 — enmienda 3: la subida de imagen en marca fallaba por el CAP de dimensiones, no por formato.** El log de Vercel (`PUT /api/brand` 422, UA Android) mostró que el rechazo era en el GUARDADO (`saveBrand`→`normalizeImage`), no en el presign. Causa raíz: `normalizeImage` (compartida marca/sello/catálogo) **rechazaba toda imagen > 2048×2048** (`limitInputPixels: 2048²` + hard-reject) → cualquier foto de cámara de teléfono (~12MP, 4000×3000) daba 422, aunque el `resize(fit:inside,2048)` de abajo existía para achicarla. **Fix:** `MAX_INPUT_PIXELS=50MP` (cubre teléfonos, acota bombas), se quita el hard-reject y el `resize` **achica** las grandes a ≤2048. Afecta las 3 superficies (correcto). Test `brand.test.ts` re-encuadrado (antes fijaba el rechazo >2048; ahora afirma downscale 4000×3000→2048×1536). Ayuda de marca "hasta 2048 px"→"se ajusta a 2048 px". Gates: typecheck 3/3, lint, unit **159**, build 3/3, prettier. Cambio en función compartida y sensible (guard de bomba de descompresión) — inline por ser QA-blocking; el bound de 50MP es el tradeoff DoS/cobertura. **Pendiente: commit+push + re-QA del owner con la misma foto que falló.** — Base: **spec 0039 — enmienda 2 (alias `image/jpg`).** El QA del owner confirmó el card del instructivo iOS (fix 1) pero un `.jpg` seguía sin subir. Causa: `image/jpg` (MIME **no-IANA** que varios selectores de Android/galerías de terceros reportan para `.jpg`; el registrado es `image/jpeg`) no estaba en `ACCEPTED_IMAGE_CONTENT_TYPES` — `sharp` lo decodifica sin problema, el bug era el gate de content-type antes de llegar al server-prep. Agregado como alias en `lib/image-formats.ts` (fuente única, así que sello+catálogo+marca lo ganan a la vez) y sumado a `MOBILE_TYPES` del guard (`upload-image-formats.test.ts`, ahora **15 tests**). Gates: typecheck 3/3, unit **158**, build 3/3, prettier. **Pendiente: commit+push + re-QA del owner con un `.jpg` real.** — Base: **spec 0039 — 2 fixes post-QA del owner aplicados (branding coherente + formatos de imagen en marca).** (1) **Card del instructivo iOS con acento de marca se veía roto** (fondo/borde azules fijos chocaban con acentos arbitrarios): ahora deriva fondo/borde/tinta del acento con helpers puros `tint`/`shade` en `lib/brand-color.ts` (unit-testeados; con el azul neutro reproducen la paleta previa → `/wallet` intacto). (2) **Bug de formatos en marca/backoffice**: subir HEIC/HEIF (foto Android/iPhone) daba "no compatible" — marca tenía la lista angosta hardcodeada en 3 lugares en vez de la fuente única `lib/image-formats.ts` (repite el bug del sello, spec 0033 QA). Migrada a `ACCEPTED_IMAGE_*` (cliente+server+`accept` con `image/*` en touch) y blindada en `upload-image-formats.test.ts` (guard ahora cubre sello+catálogo+marca; mistake→rule reforzado en CLAUDE.md). Gates: typecheck 3/3, lint, unit 155, build 3/3, prettier. Fixes UI/backoffice inline (no requieren revisor independiente — post-QA de features ya con PASS). **Pendiente: commit+push + re-QA del owner** (subir foto de teléfono en marca; ver el instructivo iOS con acento no-default legible). — Base: **spec 0039 (branding de la landing de enrolamiento) IMPLEMENTADA con PASS de revisor independiente.** Completa la superficie pública de la 0028 con el branding del negocio (spec 0025). Flujo `AGENT-WORKFLOW.md`: implementador → **revisor independiente PASS** (gates + integración Neon por su cuenta). `getEnrollLanding` (`server/consumer/enrollment.ts`) devuelve ahora `businessId`, los 3 colores de marca, `logoVersion`, `hasLogo` — **sin filtrar `logoObjectKey`** (destructuring + test `not.toHaveProperty('logoObjectKey')`, patrón anti-fuga R2). La landing (`enroll/[programId]/page.tsx`) muestra el logo por la ruta pública existente `/api/public/brands/{businessId}/logo?v={logoVersion}` (sin exponer la clave R2), con fallback al `<h1>{businessName}</h1>` si no hay logo. Botón "Sumarme al programa" en `brandPrimaryColor` con texto por luminancia (helper puro nuevo `lib/brand-color.ts` `readableTextColor`, unit-testeado — **no hay React Aria en el repo**). `PushPrompt`/`IosInstallHint` suman prop OPCIONAL `accentColor`: la confirmación pasa el color de marca, **`/wallet` NO lo pasa → queda neutro** (arco separado 0031, sin tocar). Sin migración ni secreto nuevo. Gates del revisor: **typecheck 3/3, lint, build 3/3, prettier, test 216/216** con la suite nueva `consumer-enrollment-landing.neon.integration` corriendo de verdad (3 tests contra Neon en rama efímera `br-solitary-brook-axf7xzqt`, auto-expira 2026-08-18). **Pendiente: commit+push (este turno) + deploy Vercel + QA visual del owner** (negocio con logo/color no-default → ver logo + botón branded legible; negocio sin logo → nombre). **Specs pendientes activas: 0031 (dashboard rico de `/wallet` + landing en vivo), 0032 (recuperación de cuenta OTP), 0023 (búsqueda de locales, en revisión).** — Base: **spec 0038 (ruteo de notificación por clase, ADR 0040) IMPLEMENTADA con PASS de revisor independiente.** Resuelve el duplicado hallado en el QA iOS real (pase de Apple + Web Push a la vez por una acreditación). Flujo `AGENT-WORKFLOW.md` completo: implementador → **revisor independiente PASS** (corrió los gates + integración Neon por su cuenta). Cambio: el transporte lo decide la **`class`** del aviso, no el fan-out a todo. `transactional` → **solo wallet** (Apple/Google); si el consumidor no tiene **pase alcanzable** (`consumerHasReachableWallet`: device APNs de pase Apple **o** pase Google) → **fallback a Web Push**; nunca coexisten (helper puro `planTransports` lo garantiza por construcción). `campaign` conserva el fan-out provisional (sin filas en prod). Se threadea `class` en `claimRow`/`deliverClaimed`/`deliverTransports`. **Opt-in por plataforma en la confirmación del enroll:** iOS → `IosInstallHint` (decoplado del VAPID); Android/desktop → `PushPrompt` (botón "Activar notificaciones" → suscripción, único modo de que el fallback de Android sea real; el permiso se pide en la pestaña normal, Android no exige PWA). `vapidPublicKey` cableado desde `page.tsx`. **Sin migración ni secreto nuevo** (`class` y las tablas ya existían). Gates del revisor: **typecheck 3/3, lint, build 3/3, prettier, test 207/207** con las 4 suites Neon corriendo de verdad (0 skipped) en rama efímera `br-solitary-brook-axf7xzqt` (auto-expira 2026-08-18): `wallet-push-routing` 3/3, `web-push` 6/6, `wallet-push` 6/6, `wallet-push-worker` 7/7. Test existente `web-push.neon.integration` reencuadrado transaccional→`campaign` (justificado por ADR 0040, sin perder cobertura del fan-out); test nuevo `wallet-push-routing.neon.integration` (split por hook file-size). **Pendiente: commit+push (este turno) + deploy Vercel + QA manual del owner** (iPhone con pase → una sola notif; Android sin pase pero con permiso → Web Push; Android con pase → banner de Google). — Base: **spec 0037 (Web Push) — QA iOS: Web Push diagnosticado y blindado (enmienda post-QA 3).** El Web Push no llegaba en iOS (el wallet sí). Diagnóstico por SQL en prod (`red-violet-38772073`): suscripción creada OK (`web_push_subscription`, platform `ios`), fila de cola `sent`, pero `last_error = "webpush: push service responded 403"` — Apple rechazaba la **auth VAPID**. Causa raíz: `WEB_PUSH_VAPID_SUBJECT` cargado como **email pelado** (`hola@nocodecompany.co`) sin `mailto:` (RFC 8292 §2.1 lo exige); el par de claves era válido (pública derivada de la privada por ECDH = match). **Fix config (owner, en curso):** `mailto:hola@nocodecompany.co` en Vercel + redeploy. **Fix estructural (mistake→rule, este commit):** `normalizeVapidSubject()` en `push/vapid.ts` antepone `mailto:` a cualquier subject sin esquema → una env var mal cargada nunca deshabilita el canal en silencio; test unitario nuevo (**unit 137**). Gates: typecheck 3/3, lint, unit 137, prettier. **Pendiente: tras el redeploy del owner, re-QA — disparar acreditación y verificar `last_error IS NULL` + notificación en el iPhone.** — Base: **spec 0037 (Web Push) — QA iOS en curso; enmienda post-QA 2 (UX del instructivo iOS) aplicada.** Sobre el instructivo "añadir a inicio" y la confirmación del enroll, tras feedback del owner: (1) **quitado el botón "Abrir Compartir"** de `ios-install-hint.tsx` — usaba `navigator.share()`, que abre la hoja de compartir del *contenido* (mandar-un-link), que NUNCA incluye "Añadir a pantalla de inicio" (esa acción vive solo en el menú de Safari, no hay API iOS para plantar el ícono); el botón confundía. (2) **paso a paso rehecho**: beneficio fuerte arriba (un toque = QR + avisos + beneficio esperando) + 3 pasos numerados con glifos de Compartir/Añadir dibujados como **SVG inline** (sin assets; swapeables por capturas reales luego). (3) **quitada la sección "Tus programas"** de `(consumer)/wallet/page.tsx` (+ query de memberships e imports muertos). UI-only; gates: typecheck 3/3, lint, unit 136, build 3/3, prettier. **Falta re-QA del owner en iPhone real + `git push`.** — Base: **spec 0037 (Web Push) — QA iOS en curso; enmienda post-QA aplicada.** Durante el QA en iPhone real el owner notó que el instructivo "añadir a inicio" no salía en la pantalla "¡Listo!" del enroll (Safari post-registro) — estaba solo en `/wallet` detrás del link "Ver mi tarjeta", contra el ADR 0039 §2(1). **Fix (UI-only):** componente presentacional reusable `(consumer)/ios-install-hint.tsx` (+ helper `isIosSafariBrowser()`), renderizado inline en `enroll-form.tsx` (screen `done`) para iOS Safari junto a los botones de Wallet (escape hatch); `push-prompt.tsx` reusa el mismo componente. Gates: typecheck 3/3, lint, unit 136, build 3/3 (sin server/DB → sin integración Neon). **Falta re-QA del owner** (iOS: ver el instructivo en la confirmación + instalar + suscribir + notif; Android completo) para marcar el último casillero residual del DoD. — Base: )
...
| 33 | Web Push (notificaciones de navegador, iOS + Android) | 0037 | **implementado — pendiente de revisor independiente** | **IMPLEMENTADO (2026-08-16), NO marcado `hecho` (falta revisor).** Dominio `server/push/*` (VAPID JWT ES256, cifrado RFC 8291 `aes128gcm` con `node:crypto` **verificado vs vector del Apéndice A**, canal `webpush` real/`fake`, `subscriptions.ts` con upsert/purge/fan-out + DTO sin keys), tabla `web_push_subscription` (`schema/web-push.ts`), fan-out `wallet/push-transports.ts` (`deliverTransports` = apple+google+webpush; cooldown cuenta UNO), `rotate.ts` purga suscripciones, `POST /api/public/push/subscribe` (sesión 0028, asocia al consumidor de la sesión, sin keys en la respuesta), PWA (`public/sw.js` estático + manifest dinámico `wallet/manifest.webmanifest/route.ts` con start_url=`/c/[webViewToken]`), UI `push-prompt.tsx` (iOS Safari no suscribe: instructivo + escape hatch; iOS PWA/Android piden permiso). Gates: typecheck 3/3, lint, unit 136, build 3/3, integración Neon 19/19; migración **`0023`** (NO 0022 — ya existía) aditiva aplicada+verificada en rama efímera; NO en prod (paso del orquestador post-PASS). Residuales: manifest dinámico (no estático) por el token per-consumidor del ADR 0039 §5; **quota de ramas Neon excedida** (owner debe borrar ~10 ramas viejas); QA real iOS/Android; secreto VAPID en Vercel. — Notas de diseño previas: **⚠️ DIRECTIVA DEL OWNER (2026-08-15, post-QA 0033): la 0037 se reencuadra para incluir iOS TAMBIÉN, no solo Android.** El diseño se discute con el owner al inicio de la próxima sesión ANTES de cerrar la spec. Repensar: en iOS Web Push exige "Agregar a pantalla de inicio" (PWA, iOS 16.4+) — hay que decidir cómo se pide esa instalación y cómo convive con el pase de Apple (que ya notifica). El ADR 0038 (transportes wallet/webpush) sigue válido pero su punto "Web Push solo Android" hay que revisarlo. **Primer paso próxima sesión: conversar el alcance iOS+Android con el owner y actualizar ADR 0038 + spec 0037.** — Notas de diseño previas: **Spec BORRADOR (2026-08-15) + ADR 0038.** 2º transporte de notificación junto al pase (ADR 0038): Web Push (Push API + Service Worker + VAPID) **solo Android**, para dar notificación de **contenido controlado** donde el pase de Google muestra banner genérico (límite hallado en el QA de la 0033). iOS lo cubre el pase de Apple (notifica rico) → Web Push en iOS diferido (exige PWA en pantalla de inicio). Tabla `web_push_subscription` + canal `webpush` + service worker + fan-out del transaccional a los transportes del consumidor; **transporte provisionado para que las campañas futuras elijan wallet/browser** (distinta tasa de conversión: pase ~90% open/contenido acotado en Android; web push ~5–6% opt-in, ~34–65% open/contenido rico). Migración aditiva `0022`. **Primer paso: cerrar la sección «Abierto» con el owner** (dónde vive el prompt, forma del transporte en la cola, paquete `web-push` vs `node:crypto`). Depende de 0033 (hecha); serializar con 0031. |
