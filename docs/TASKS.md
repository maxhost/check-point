...
Ultima actualizacion: 2026-08-15 (**spec 0030 acreditación en mostrador: IMPLEMENTADA con PASS de revisor independiente** — `AGENT-WORKFLOW.md`). Dominio `server/counter/*` (core/resolve/grant/orders) + rutas `api/counter/{resolve,grant}` (auth `merchant_auth` scopeada por `business_membership`) + UI móvil `/backoffice/counter` (scanner `BarcodeDetector`+`jsqr`, venta detallada/carrito o rápida/importe+nota, Confirmar deshabilitado al 1er tap). **Otorgamiento atómico** vía un CTE guardado (`persistGrant`: bump del saldo con guard `NOT EXISTS(order)` + `INSERT … ON CONFLICT DO NOTHING`) e **idempotente** por `unique(business_id, client_request_id)` — sin doble-bump bajo concurrencia real (el revisor lo verificó con una sonda 8-way). **Anti-fuga** blindada (DTOs allow-list; ningún `qr_token`/`token_hash`/`web_view_token`/`auth_token_hash`). Migración aditiva **`0020_harsh_venus`** aplicada y verificada por SQL en **prod** (21 migraciones; `core.order`/`order_item` + `points_balance`/`stamps_count` con checks; `core`(22)/`consumer`(5)/`merchant_auth`(4) intactos). Paquete nuevo **`jsqr`** (fallback de decodificación; store re-warmeado por el orquestador con red). Gates: typecheck 3/3, lint, prettier, **unit 106/44-skip** (7 nuevos de counter), build 3/3, integración Neon **8/8** counter + **25/25** regresión (0028/0034/0036/wallet) en rama efímera `br-late-wind-axuztnet` (auto-expira 2026-08-17). **Residual:** QA manual del owner en teléfono (HTTPS+cámara) sobre el deploy; falta `git push` a `main` (espera OK del owner). **La próxima rebanada del camino A es la spec 0031** (notificación + landing en vivo + dashboard "Ver mis programas") — lee las tablas `order`/`order_item` de 0030.)
...
| 24 | Acreditación en mostrador (consola web móvil, puntos/sellos por reglas) + auto-enrolamiento por escaneo | 0030 | hecho | **IMPLEMENTADA (2026-08-15) con PASS de revisor independiente.** Dominio `server/counter/*` (core/resolve/grant/orders) + rutas `api/counter/{resolve,grant}` (+ `_auth.requireOperator`) + UI `/backoffice/counter` (page server + counter-console/stages/sale-forms/qr-scanner; scanner `BarcodeDetector`+`jsqr`; venta detallada/rápida; Confirmar deshabilitado al 1er tap). Otorgamiento **atómico** vía CTE guardado (`persistGrant`: bump con guard `NOT EXISTS(order)` + `ON CONFLICT DO NOTHING`) e **idempotente** por `unique(business_id, client_request_id)` — sin doble-bump bajo concurrencia real (verificado por el revisor con sonda 8-way). Anti-fuga: DTOs allow-list, ningún `qr_token`/`token_hash`/`web_view_token`/`auth_token_hash` serializado (test). Migración aditiva **`0020_harsh_venus`** aplicada y verificada por SQL en prod (21 migraciones; `core.order`/`order_item` + saldo en `program_membership`; `core`(22)/`consumer`(5)/`merchant_auth`(4) intactos). Paquete nuevo `jsqr` (fallback de decodificación). Gates: typecheck 3/3, lint, prettier, **unit 106/44-skip**, build 3/3, integración Neon **8/8** counter + **25/25** regresión en rama efímera. **Residual:** QA manual del owner en teléfono (HTTPS+cámara). La notificación (paso 6) = spec 0031. **Modelo:** saldo por membresía (`points_balance`/`stamps_count`) + `core.order`/`order_item` owner-facing (ledger de auditoría). **Disjunta: no** — crea las tablas que la spec 0031 lee → serializar 0031 después. |
