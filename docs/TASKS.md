...
| 19 | Implementar marca real y assets R2 | 0025 | hecho | Nombre, colores, timezone y logo privado procesado/servido desde R2; reemplaza el mock de Marca. Gates locales verdes, pusheado a `main` (`b576a4b`) y QA manual en vivo confirmado por el owner el 2026-08-12. Spec cerrada a `implementada` a pedido del owner. 2026-08-13: la concurrencia queda **a futuro** (control optimista `409` en código; prueba con >1 owner diferida) y corre una **revisión independiente** de 0025 para llevarla al mismo estándar que 0024; el endurecimiento (magic bytes/sharp, dims, authz) ya está en código. |
| 20 | Diseño de sello del programa de fidelización en R2 | 0026 | pendiente | Input para subir el diseño de la imagen del sello (modalidad Sellos): PNG/JPEG/WebP, aplanado sin transparencia (a confirmar), borrado diferido a Guardar (igual que marca), reutilizando el pipeline R2 de 0025. Spec `en curso`: cerrar tras confirmar transparencia + ADR de extracción del módulo de assets, antes de tocar código. |
