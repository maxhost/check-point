...
Ultima actualizacion: 2026-08-14 (spec 0036 mecánica + premios: **IMPLEMENTADA con PASS de revisor, migración `0019` en prod, pusheada, y QA del owner en vivo COMPLETO** — 3 refinamientos de UX del wizard shippeados: copy del $-equivalente, auto-sugerencia del costo en puntos desde el precio del producto, y preview de valor más potente. **Prod quedó limpio** (se borraron los 5 programas de prueba + membresías a pedido del owner, para probar de cero). **La próxima feature es la spec 0030 (acreditación en mostrador)**: es `borrador`/stub y el primer paso es **cerrarla con el owner**; 0036 ya resolvió parte de su «Abierto» (ver bloque abajo). Catálogo 0034/0035 implementado, migraciones 0017/0018/0019 en prod).
...
| 24 | Acreditación en mostrador (consola de staff, puntos/sellos por reglas) + auto-enrolamiento por escaneo | 0030 | pendiente (bloqueada por 0036) | Stub/borrador. 3ª rebanada. **Bloqueada hasta implementar la spec 0036** (mecánica + premios: sin ella no hay nada que otorgar ni canjear). Escanear QR del consumidor → carrito → otorgar puntos/sellos con la mecánica de 0036 (`floor(total/Y)×X`; Sellos por compra o por monto) → **canjear** premios de `loyalty_reward` (descontar puntos / resetear tarjeta, atómico + auditoría). **Hereda del ADR 0033:** resolución del `qr_token` global desambiguada por el negocio que escanea + **auto-enrolamiento por escaneo** (alta on-the-fly de un consumidor que ya existe pero no es miembro; términos accesibles en "Ver mis programas"). Prerequisitos duros: **catálogo (spec 0034, cerrada)** + **mecánica/premios (spec 0036, cerrada)**. Depende de 0028. |
...
| 2026-08-14 | **PRÓXIMA FEATURE — spec 0030 (acreditación en mostrador), punto de retorno** | **Estado: `borrador`/stub; primer paso = cerrarla con el owner** (protocolo `AGENT-WORKFLOW.md`). **Dependencias duras YA satisfechas:** 0028 (identidad + `qr_token` + `program_membership`), 0034 (catálogo de productos), 0029 (pase de Wallet con el QR), y **0036 (mecánica + premios)**. **0036 resolvió parte del «Abierto» del stub 0030:** la «equivalencia $X = Y puntos» = la tasa de acumulación (`accrual_grant`/`accrual_block_amount`), y las «reglas de sello 1-por-compra / 1-por-cada-$X» = `accrual_mode` (`per_purchase`/`per_amount`); el otorgamiento ya está fijado en `computeAccrual` (server `loyalty-program/accrual.ts`) y los premios en `core.loyalty_reward`. **Lo que 0030 SÍ debe diseñar/cerrar (queda abierto):** (a) modelo de **saldo de puntos + progreso de sellos por membresía** (tablas nuevas en `consumer`, con auditoría por evento estilo 0024); (b) **ejecución del canje** (descontar puntos / resetear la tarjeta de sellos, atómico + auditoría) usando las definiciones de `loyalty_reward`; (c) **resolución del `qr_token` → consumidor** desambiguada por el negocio que escanea + **auto-enrolamiento por escaneo** (ADR 0033); (d) **autorización del staff** sobre el QR (qué ve, qué puede otorgar; ADR 0002/0007); (e) **UI de carrito** con productos del catálogo → `total` → `computeAccrual`. **Prod está limpio de programas** (crear uno nuevo con mecánica para probar 0030). |
