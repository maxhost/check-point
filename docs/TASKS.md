...
Ultima actualizacion: 2026-08-15 (**spec 0030 acreditación en mostrador: CERRADA con el owner**, lista para implementar. Se afinó el comportamiento punta a punta: **consola web móvil** en una URL del backoffice —cámara del teléfono, guardable como favorito— que escanea el QR del consumidor, resuelve/auto-enrola su membresía y acredita puntos/sellos por **venta detallada** (carrito del catálogo) o **venta rápida** (importe + nota, **inmutable**). **Solo acreditación** —el canje salió del alcance: es otra URL/mecánica/feature—. Modelo: **saldo por membresía** (`points_balance`/`stamps_count` en `consumer.program_membership`) + **orden owner-facing en `core`** (`order`+`order_item` con snapshot) que **ES el ledger de auditoría** (analítica del negocio, no se expone al consumidor); otorgamiento **atómico** vía `computeAccrual` (0036) e **idempotente en dos capas** (DB `unique (business_id, client_request_id)` + UI que deshabilita Confirmar al primer tap). QR: `BarcodeDetector` nativo + fallback JS. **La notificación al consumidor (paso 6) es la spec 0031** —0030 registra y emite; 0031 entrega—. Todas las dependencias duras satisfechas: 0028/0029/0034/0036 implementadas, migraciones 0014–0019 en prod. **Próximo paso: implementar 0030** con `AGENT-WORKFLOW.md`.)
...
| 24 | Acreditación en mostrador (consola web móvil, puntos/sellos por reglas) + auto-enrolamiento por escaneo | 0030 | pendiente (spec CERRADA) | **PRÓXIMA FEATURE — spec CERRADA (2026-08-15), lista para implementar.** Consola web móvil `/backoffice/counter` (cámara, bookmarkable, auth `merchant_auth`): escanea el QR → resuelve/auto-enrola la membresía del negocio → toggle **venta detallada** (carrito del catálogo 0034 → total) / **venta rápida** (importe + nota, inmutable) → `computeAccrual` (0036) → otorga. **Solo acreditación** (el canje es otra feature/URL, fuera del alcance). Modelo nuevo: **saldo por membresía** (`points_balance`/`stamps_count` en `consumer.program_membership`, migración aditiva) + **orden owner-facing en `core`** (`order`+`order_item` con snapshot) que ES el ledger de auditoría. Otorgamiento **atómico** (patrón CTE 0024/0028) e **idempotente en dos capas** (DB unique `(business_id, client_request_id)` + UI deshabilita Confirmar al primer tap). QR: `BarcodeDetector` + fallback JS (**re-warm del store de pnpm antes de codear** — suma paquete). Dominio `server/counter/*`, rutas `api/counter/{resolve,grant}`, UI `app/backoffice/counter/*`. La notificación (paso 6) = spec 0031 (0030 emite el evento). **Disjunta: no** — crea las tablas que 0031 lee → serializar 0031 después. Depende de 0028/0029/0034/0036 (todas hechas). Implementar con `AGENT-WORKFLOW.md` (rama Neon efímera + revisor independiente antes de `implementada`). |
