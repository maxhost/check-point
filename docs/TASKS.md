...
Ultima actualizacion: 2026-09-08 (**SPEC 0056 IMPLEMENTADA CON PASS DE REVISOR INDEPENDIENTE — el bug de doble
acreditacion esta arreglado en el working tree, COMMITEADO Y SIN PUSHEAR (o sea: TODAVIA VIVO EN PROD hasta que
se pushee; el deploy ES el fix, no hay migracion). El fix es la linea que la spec predijo (fuera el `ON CONFLICT
...
bug). **PROXIMO PASO: pushear (deploy = fix) + verificar por SQL en prod que ningun saldo difiera de la suma de
sus ordenes; despues, la spec 0055 (canje, tarea 44).** Nota: el implementador murio con el proceso y no dejo
handoff — el orquestador re-verifico todo antes de mandar a revision.)
...
**PROXIMO PASO AL RETOMAR: la spec 0056 ya esta implementada (PASS). Falta (a) pushear —el deploy es el fix, el
bug sigue vivo en prod hasta entonces— y (b) correr la verificacion post-deploy por SQL. Despues, implementar la
spec 0055** (canje, tarea 44), que ya esta `cerrada` y nace con `withDbTransaction`: no hace falta cerrar nada
con el owner para arrancarla.
