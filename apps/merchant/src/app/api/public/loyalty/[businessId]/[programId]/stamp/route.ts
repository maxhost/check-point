...
export async function GET(
  ...
  try {
    ...
    const program = await stampForPublicProgram(businessId, programId, version);
    if (!program) return new NextResponse(null, { status: 404 });
    const object = await getPrivateObject(
      `${program.stampImageObjectKey}/stamp.${webp ? "webp" : "png"}`,
    );
