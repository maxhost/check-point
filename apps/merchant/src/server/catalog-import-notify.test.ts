...
/**
 * Spec 0091 §7 / ADR 0082 §12 — EL AVISO, **una sola vez, AL NEGOCIO y DESPUES del resultado
 * final**. Ya no hay un «listo para revisar» a mitad de camino: o el menu quedo en el
 * catalogo, o el import fallo.
 *
...
describe("aviso por email de la importación (spec 0091 §7)", () => {
  ...
  it("el cuerpo NO lleva nombres de productos ni fragmentos del menú (§9)", async () => {
    estado.filas = [[reclamada], [{ email: "duena@example.test" }]];
    await notifyImportFinished(IMPORT_ID, "accepted", canal);
