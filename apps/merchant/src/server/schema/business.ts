...
export const businesses = core.table(
  "business",
  {
    ...
    /** Spec 0069 §D1 / ADR 0070 §7: categoria del negocio, guardada como el `gcid:`
     * CRUDO de Google Business Profile (no un enum propio) para que el dia que se
     * apruebe el Basic API Access la lista curada de `lib/business-categories.ts` se
     * reemplace por la API **sin migrar un dato**.
     * El DEFAULT **no es la regla de producto** —esa es «obligatoria en la ruta de
     * alta», `400` sin `categoryGcid` valido— sino el mismo patron que `slug`:
